...
output$downloadPDF <- downloadHandler(
  ...
  content = function(file) { withProgress(message = 'This takes a minute or two',{
    ...
    if (!is.null(saved_plots$SampleCorrelation) & ("Sample Correlation Plot" %in% plots_checked) ){
      draw(saved_plots$SampleCorrelation, merge_legend=T,auto_adjust = FALSE)
      Np=Np+1
    }
...
observe({
  ...
  if (!is.null(saved_plots$SampleCorrelation)){
    summary=c(summary, 'Sample Correlation Plot')
  }
...
output$downloadSVG <- downloadHandler(
  ...
  content = function(file) { withProgress(message = 'This takes a minute or two',{ 
    ...
    if (Np==0 & !is.null(saved_plots$SampleCorrelation) & ("Sample Correlation Plot" %in% plots_checked) ){
      draw(saved_plots$SampleCorrelation, merge_legend=T,auto_adjust = FALSE)
      Np=Np+1
    }
    #Dendrograms
